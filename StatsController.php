...
class StatsController
{
    ...
    {
        $matches = $this->storage->getData($this->storage->getMatchesDataFile());
    ...
    public function getH2HStats(): void
    {
        ...
        if (!$player1Name || !$player2Name) {
            $this->jsonResponse(['success' => false, 'message' => 'Two player names are required.']);
            return;
        }

        $allMatches = $this->storage->getData($this->storage->getMatchesDataFile());
